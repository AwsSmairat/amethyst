...
import 'package:amethyst/features/driver_cash/presentation/widgets/register_driver_cash_sheet.dart';
import 'package:flutter/material.dart';

/// بطاقة رصيد الأموال في لوحة السائق.
class DriverCashBalanceDashboardCard extends StatefulWidget {
  const DriverCashBalanceDashboardCard({super.key});

  @override
  State<DriverCashBalanceDashboardCard> createState() =>
      _DriverCashBalanceDashboardCardState();
}

class _DriverCashBalanceDashboardCardState
    extends State<DriverCashBalanceDashboardCard> {
  bool _loading = true;
  String? _error;
  DriverCashBalanceSnapshot _snapshot =
      const DriverCashBalanceSnapshot(todayAmount: 0, yesterdayAmount: 0);

  @override
  void initState() {
    super.initState();
    DriverCashListRefresh.onRefreshRequested = _load;
    _load();
  }

  @override
  void dispose() {
    if (DriverCashListRefresh.onRefreshRequested == _load) {
      DriverCashListRefresh.onRefreshRequested = null;
    }
    super.dispose();
  }
  ...
  Future<void> _openRegisterSheet() async {
    await showRegisterDriverCashSheet(context);
    await _load();
  }
