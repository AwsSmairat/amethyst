...
Future<bool> showRegisterStationCashSheet(BuildContext context) async {
  final bool? saved = await showModalBottomSheet<bool>(
    ...
    builder: (BuildContext context) => Padding(
      ...
      child: const _RegisterStationCashBody(),
    ),
  );
  return saved == true;
}

class _RegisterStationCashBody extends StatefulWidget {
  const _RegisterStationCashBody();

  @override
  State<_RegisterStationCashBody> createState() =>
      _RegisterStationCashBodyState();
}

class _RegisterStationCashBodyState extends State<_RegisterStationCashBody> {
  ...
  Future<void> _submit() async {
    ...
    try {
      await sl<SetStationCashBalanceUseCase>()(amount: todayBalance);
      if (!mounted) {
        return;
      }
      StationCashListRefresh.request();
