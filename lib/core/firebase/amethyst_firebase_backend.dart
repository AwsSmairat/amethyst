...
final class AmethystFirebaseBackend {
  ...
  Future<Map<String, dynamic>> listUsers({int page = 1, int limit = 100}) async {
    ...
    List<Map<String, dynamic>> all = snap.docs.map(mapUserDoc).toList(growable: false);
    if (actor['role'] == 'admin') {
      all = all
  ...
  Future<Map<String, dynamic>> getDashboardSuperAdmin() async {
    ...
    final QuerySnapshot<Map<String, dynamic>> users =
        await _db.collection(FirestorePaths.users).get();
    ...
    for (final QueryDocumentSnapshot<Map<String, dynamic>> u in users.docs) {
      final String? role = u.data()['role'] as String?;
      if (role == 'admin') {
        admins++;
      }
      if (role == 'driver') {
    ...
    final double expensesToday = await _sumExpenses(day.start, day.end);
    ...
    final double monthlyExpenses = await _sumExpenses(month.start, month.end);
